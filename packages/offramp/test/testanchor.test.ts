...
describe("TestAnchorOffRamp (offline)", () => {
  ...
  it("status() throws a typed OffRampJobNotFoundError for an unknown job id, not an anonymous Error", async () => {
    ...
    await expect(offramp.status("no-such-job")).rejects.toBeInstanceOf(
...
describe.skipIf(!process.env.RUN_LIVE_ANCHOR_TESTS)(
  "TestAnchorOffRamp (live)",
  () => {
    const telemetry: {
      rate?: string;
      quoteLatencyMs?: number;
      withdrawLatencyMs?: number;
    } = {};
