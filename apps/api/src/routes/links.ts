...
export function linkRoutes(c: Container, strictRateLimit: MiddlewareHandler): Hono<{ Variables: AuthVariables }> {
  ...
  // Create a payment link. (links:write)
  app.post("/", strictRateLimit, auth, requireScope("links:write"), idempotent, async (ctx) => {
  ...
  // List the seller's links. (links:read)
  app.get("/", auth, requireScope("links:read"), async (ctx) => {
  ...
  // Seller-initiated cash-out to local currency. (offramp:initiate — moves
  // money, opt-in and off by default.)
