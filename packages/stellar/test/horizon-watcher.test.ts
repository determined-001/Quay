...
describe("HorizonWatcher.fetchSince", () => {
  it("requests join=transactions so the memo lookup costs zero extra Horizon calls", async () => {
    ...
    // Constructor takes a Horizon URL and builds its own Horizon.Server; swap
    // in the fake afterward rather than hitting the network in a unit test.
    const watcher = new HorizonWatcher("https://unused.example");
    (watcher as any).server = server;
