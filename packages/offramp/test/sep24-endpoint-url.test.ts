...
describe("endpointUrl", () => {
  it("keeps the transfer server's path prefix", () => {
    ...
    ).toBe("https://testanchor.stellar.org/sep24/transaction");
  });
  ...
  it("is what `new URL(path, base)` gets wrong", () => {
    ...
    ).toBe("https://testanchor.stellar.org/transaction");
    expect(
      endpointUrl(
        "https://testanchor.stellar.org/sep24",
        "transaction",
  ...
  it("tolerates a trailing slash on the base and a leading slash on the path", () => {
    ...
    expect(
      endpointUrl("https://anchor.example/sep24/", "transaction").toString(),
    ).toBe(expected);
    expect(
      endpointUrl("https://anchor.example/sep24", "/transaction").toString(),
    ).toBe(expected);
  ...
  it("still works when the transfer server is the bare origin", () => {
    expect(
      endpointUrl("https://anchor.example", "transaction").toString(),
    ).toBe("https://anchor.example/transaction");
  });

  it("preserves a nested path prefix", () => {
    expect(
      endpointUrl(
        "https://anchor.example/api/v2/sep24",
        "transaction",
      ).toString(),
    ).toBe("https://anchor.example/api/v2/sep24/transaction");
  });
});
