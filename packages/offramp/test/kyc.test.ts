...
const NAME: KycFieldSpec = {
  name: "first_name",
  type: "string",
  optional: false,
};
const EMAIL: KycFieldSpec = {
  name: "email_address",
  type: "string",
  optional: false,
};
const MIDDLE: KycFieldSpec = {
  name: "middle_name",
  type: "string",
  optional: true,
};
...
describe("missingRequiredFields", () => {
  it("names exactly the missing required fields", () => {
    ...
    expect(missingRequiredFields([NAME, EMAIL], { first_name: "Ada" })).toEqual(
      ["email_address"],
    );
  });
  ...
  it("treats a whitespace-only value as missing, not fabricated", () => {
    expect(missingRequiredFields([NAME], { first_name: "   " })).toEqual([
      "first_name",
    ]);
  });
  ...
  it("never requires an optional field", () => {
    expect(
      missingRequiredFields([NAME, MIDDLE], { first_name: "Ada" }),
    ).toEqual([]);
  });
