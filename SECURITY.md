...
| `JWT_SECRET` | *(not yet used anywhere in this codebase - no auth/session layer exists yet; see README's "Auth: Not implemented")* | Once an auth layer lands and actually reads this, rotating it invalidates every outstanding token immediately - plan for a brief mixed-validity window (old and new secrets both accepted) if sessions need to survive a rotation, or accept forcing every user to re-authenticate. Documented here ahead of time per issue 8.5, not because it's in use today. |
